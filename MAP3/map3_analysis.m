...
filename = 'D:\Dropbox (MIT)\Spring 2017\NVBOTS Dropbox\Summer\error_mapping_artifact\metrology_data\MAP3\parsed_raw_data.csv';
delimiter = ',';
startRow = 2;
...
metrology_data.FeatureNumber = (1:25)';
...
%% Import the data
[~, ~, raw] = xlsread('D:\Dropbox (MIT)\Spring 2017\NVBOTS Dropbox\Summer\error_mapping_artifact\artifact_R3 - Variable Pattern Table - VarPattern.xlsx','Sheet1','A3:E83');
...
design_data.FeatureNumber = (1:25)';
...
x = reshape(design_data.DesignX,5,5);
y = reshape(design_data.DesignY,5,5);
z_err = reshape(error,5,5);
figure
surf(x,y,z_err)
xlabel('X (mm)')
ylabel('Y (mm)')
...
figure
surf(x,y,reshape(parallelism,5,5))
xlabel('X (mm)')
ylabel('Y (mm)')
...
%% Fit model to error
lm_lin = fitlm([design_data.DesignX design_data.DesignY design_data.DesignZ],error,'linear', 'VarNames', {'X','Y','Z','Error'},'Intercept',false); %exclude outlier point (visibly bad column)
